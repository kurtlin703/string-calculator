...
function add(string_number) {
  ...
  if (string_number.startsWith("//")) {
    ...
    if (delimiterString.startsWith("//[")) {
      let delimitersWithoutBrackets = string_number.match(
        /(?<=\[).*(?=\])/g
      )[0];
      delimitersWithoutBrackets = delimitersWithoutBrackets.replace(
        /\]\[/g,
        "|"
      );
      delimiters = new RegExp(
        escapingRegexSpecialCharacters(delimitersWithoutBrackets),
        "g"
      );
